...
- **Mode:** PVE co-op boss mode for v1. PVP head-to-head is a later mode (keep `matchups`/`parties` tables around, unused).
...
- **Auth:** Simple shared party — invite code + display name, no passwords. Lightweight member token issued on join so actions map to a member slot.
