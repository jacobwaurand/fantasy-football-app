...
export function canAddPlayerToTeam(rosterSettings: RosterSettings, currentRoster: Player[], playerToAdd: Player): boolean {
  const positionCount = currentRoster.reduce(
    (acc, player) => {
  ...
  if (
    ...
    rosterSettings.benchSlots +
      rosterSettings.numQB +
      rosterSettings.numRB +
      rosterSettings.numWR +
      rosterSettings.numTE +
      rosterSettings.numFLEX +
      rosterSettings.numDEF +
      rosterSettings.numK
  ) {
    return false;
  }
